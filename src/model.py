...
class SimCSE(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.hard_pair_mining = args.hard_pair_mining        
    ...
    def forward(self, input_ids, attention_mask, label):
        ...
        # z1, z2 : (batch_size, hidden_size)
        z1, z2 = pooler_output[:,0], pooler_output[:,1]

        # calculate cosine similarity
        sim_matrix = self.sim(z1.unsqueeze(1), z2.unsqueeze(0))

        # set labels and loss function
        label = label.long().to(self.device)
        
        loss_fct = MultiSimilarityLoss(threshold=self.threshold,
                                       scale_pos=self.scale_pos,
                                       scale_neg=self.scale_neg,
                                       margin=self.margin,
                                       hard_pair_mining=self.hard_pair_mining,
                                       device=self.device,
                                      )
        
        loss = loss_fct(sim_matrix, label)
